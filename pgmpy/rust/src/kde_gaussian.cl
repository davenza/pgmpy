#line 1 "./pgmpy/rust/src/kde_gaussian.cl.src"
...
)
{
    ...
    uint instance = gid / (nparents_kde*nparents_kde);
...
)
{
    ...
    if (lid == 0) {
        if (instance < 10) {
            printf("final_result[%d] += exp(sums_buffer[0] (%f) - max_buffer[%d] (%f) = %f",
                    instance, sums_buffer[0], instance, max_buffer[instance],
                    exp(sums_buffer[0] - max_buffer[instance]));
        }

        final_result[instance] += exp(sums_buffer[0] - max_buffer[instance]);
    }
}
...
)
{
    ...
    for(int i = 0; i < len_evidence; i++) {
        Cj += beta[evidence_index[i]+1]*test_dataset[base_pos + ADD_BASE_RM(row, evidence_index[i], leading_dimension)];
    }
...
)
{
    ...
    for(int i = 0; i < len_evidence; i++) {
        Cj += beta[evidence_index[i]+1]*test_dataset[base_pos + ADD_BASE_CM(row, evidence_index[i], leading_dimension)];
    }
