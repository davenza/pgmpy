...
__kernel void substract_without_origin(__constant double *train_data,
                        __constant double *vec,
                        __global double *res,
                        __private uint row,
                        __private uint n_col) {
    int i = get_global_id(0);
    int c = i % n_col;
    res[i] = train_data[i*(n_col+1) + c + 1] - vec[row*(n_col+1) + c + 1];
}

/**
 * This kernel computes -0.5 * Mahalanobis distance
 * @param Ti
 * @param precision
 * @param res
 * @param uint
...
__kernel void mahalanobis(__constant double *Ti,
    ...
    for (int i = 0; i < n_col; i++) {
        tmp -= 0.5*precision[(i+1)*(n_col+1)+i+1]*Ti[idx*n_col + i]*Ti[idx*n_col + i];

        for(int j = i+1; j < n_col; j++) {
            tmp -= precision[(i+1)*(n_col+1)+j+1]*Ti[idx*n_col + i]*Ti[idx*n_col + j];
        }
    }
...
/**
 * This kernel computes -dot(Lambda[K, -K], Ti)
 * @param Ti
 * @param precision
 * @param res
 * @param uint
 * @return
 */
__kernel void dot_product_bi(__constant double *Ti,
                          __constant double *precision,
                          __global double *res,
                          __private uint n_col) {
    int idx = get_global_id(0);
    ...
    for (int i = 0; i < n_col; i++) {
        tmp -= precision[i+1]*Ti[idx*n_col + i];
    }

    res[idx] = tmp;
}

__kernel void generate_exponents_onlykde(__constant double *training_dataset,
                                __private double inv_a,
                                __global double *bi,
                                __constant double *ci,
                                __constant double *precision,
                                __private uint n_col)
{
    ...
    double tk = training_dataset[idx*n_col];

    double final_ci = ci[idx] - tk*bi[idx] - 0.5*tk*tk*precision[0];
    double final_bi = bi[idx] + tk*precision[0];

    bi[idx] = final_bi*final_bi*inv_a + final_ci;
}

__kernel void generate_exponents(__constant double *training_dataset,
                                          __private double inv_a,
                                          __global double *bi,
                                          __constant double *ci,
                                          __constant double *precision,
                                          __private double b_gaussian,
                                          __private double c_gaussian,
                                          __private uint n_col) {
    ...
    double final_ci = ci[idx] - tk*bi[idx] - 0.5*tk*tk*precision[0] - 0.5*c_gaussian;
    double final_bi = bi[idx] + tk*precision[0] - b_gaussian;
...
/**
#############################################
# Denominator - Iterate train (high memory) #
#############################################
*/

__kernel void fill_coefficients_mat_onlykde(__constant double* training_dataset,
                                    __private double inv_a,
                                    __constant double* bi,
                                    __constant double* ci,
                                    __constant double* precision,
                                    __global double* matrix,
                                    __private uint column,
                                    __private uint total_columns)
{
    uint idx = get_global_id(0);

    uint mat_idx = get_global_id(0)*total_columns + column;

    double tk = training_dataset[idx*total_columns];

    double final_ci = ci[idx] - tk*bi[idx] - 0.5*tk*tk*precision[0];
    double final_bi = bi[idx] + tk*precision[0];

    matrix[mat_idx] = final_bi*final_bi*inv_a + final_ci;
}

__kernel void fill_coefficients_mat(__constant double* training_dataset,
                                    __private double inv_a,
                                    __constant double* bi,
                                    __constant double* ci,
                                    __constant double* precision,
...
/**
##########################################
# Denominator Iterate train (low memory) #
##########################################
*/

__kernel void coefficients_checkmax_onlykde(__constant double* training_dataset,
                                    __private double inv_a,
                                    __constant double* bi,
                                    __constant double* ci,
                                    __constant double* precision,
                                 ...
                                 )
{
    ...
    double coefficient = final_bi*final_bi*inv_a + final_ci;

    max_vec[idx] = max(max_vec[idx], coefficient);
}

__kernel void coefficients_checkmax(__constant double* training_dataset,
                                    __private double inv_a,
                                    __constant double* bi,
                                    __constant double* ci,
                                    __constant double* precision,
...
__kernel void coefficients_sum_onlykde(__constant double* training_dataset,
                                   __private double inv_a,
                                   __constant double* bi,
                                   __constant double* ci,
                                   __constant double* precision,
                                   __constant double* max_vec,
...
)
{
    ...
    final_result[idx] += exp(coefficient - max_vec[idx]);
}

__kernel void coefficients_sum(__constant double* training_dataset,
                                       __private double inv_a,
                                       __constant double* bi,
                                       __constant double* ci,
                                       __constant double* precision,
