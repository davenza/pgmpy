...
)
{
    ...
    *error = Error::NoError;
    if kde_box.n >= m {
        logdenominator_iterate_test(&mut kde_box, &mut pro_que, x, precision,
                                    a_gaussian, b_gaussian, c_gaussian, result, error);
    } else {
        logdenominator_iterate_train(&mut kde_box, &mut pro_que, x, precision,
                                     a_gaussian, b_gaussian, c_gaussian, result, error);
    }
...
unsafe fn logdenominator_iterate_test(kde: &mut Box<GaussianKDE>,
                                      ...
                                      error: *mut Error)
{
    ...
    let a = 0.5*(precision_slice[0] + a_gaussian);
    ...
    let kernel_generate_exponents = pro_que
        .kernel_builder("generate_exponents")
        .global_work_size(n)
        .arg(&kde.training_data)
        .arg(a.recip())
        .arg(&bi)
        ...
        .arg(d as u32)
        .build()
        .expect("Kernel generate_exponents build failed.");
    ...
    for i in 0..m {
        ...
        kernel_sum_to_bi_ci
            .enq()
            .expect("Error while executing sum_to_bi_ci kernel.");
...
unsafe fn logdenominator_iterate_train(kde: &mut Box<GaussianKDE>,
                                       pro_que: &mut Box<ProQue>,
                                       x: *const DoubleNumpyArray,
                                       precision: *const DoubleNumpyArray,
                                       a_gaussian: double,
                                       ...
                                       error: *mut Error)
{
    ...
    match tmp_vec_buffer {
        Ok(b) => logdenominator_iterate_train_high_memory(kde, pro_que, x, precision,
                                                          a_gaussian, b_gaussian, c_gaussian,
                                                          result, &b, error),
        Err(_) => {
            // TODO: If n < 2m, is it better to iterate over the training data?
            logdenominator_iterate_train_low_memory(kde, pro_que, x, precision, a_gaussian,
                                                    b_gaussian, c_gaussian, result, error);
        }
    }
...
unsafe fn logdenominator_iterate_train_low_memory(kde: &mut Box<GaussianKDE>,
                                                  pro_que: &mut Box<ProQue>,
                                                  x: *const DoubleNumpyArray,
                                                  precision: *const DoubleNumpyArray,
                                                  a_gaussian: double,
                                                  ...
                                                  error: *mut Error)
{
    ...
    let kernel_coefficients_checkmax = pro_que
        .kernel_builder("coefficients_checkmax")
        .global_work_size(m)
        .arg(&kde.training_data)
        .arg(a.recip())
        .arg(&bi)
        ...
        .arg(d as u32)
        .build()
        .expect("Kernel coefficients_checkmax build failed.");

    let kernel_coefficients_sum = pro_que
        .kernel_builder("coefficients_sum")
        .global_work_size(m)
        .arg(&kde.training_data)
        .arg(a.recip())
        .arg(&bi)
...
unsafe fn logdenominator_iterate_train_high_memory(kde: &mut Box<GaussianKDE>,
                                                   pro_que: &mut Box<ProQue>,
                                                   x: *const DoubleNumpyArray,
                                                   precision: *const DoubleNumpyArray,
                                                   a_gaussian: double,
                                                   ...
                                                   error: *mut Error)
{
    ...
    let kernel_fill_coefficients_mat = pro_que
        .kernel_builder("fill_coefficients_mat")
        .global_work_size(m)
        .arg(&kde.training_data)
        .arg(a.recip())
        .arg(&bi)
