...
from pgmpy.estimators import K2Score, GaussianBicScore, BdeuScore, BicScore, BGeScore, CachedHillClimbing, HybridContinuousScore
from pgmpy.base import DAG


class CachedHillClimbing_ContinuousHybrid(CachedHillClimbing):
    ...
    def __init__(self, data, scoring_method=None, blacklist=[], whitelist=[], **kwargs):
        ...
        if not np.all(data.dtypes == 'float64'):
            raise ValueError("All the data columns should be continuous to learn an hybrid continuous model.")

        if not isinstance(scoring_method, HybridContinuousScore):
            raise ValueError("Wrong scoring method for hybrid continuous models. It should be of type HybridContinuousScore.")

        if scoring_method is None:
            self.scoring_method = HybridContinuousScore(data, **kwargs)
        ...
        super(CachedHillClimbing_ContinuousHybrid, self).__init__(data, **kwargs)
