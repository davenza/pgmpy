...
from pgmpy.estimators import StructureEstimator, K2Score, GaussianBicScore, BdeuScore, BicScore, BGeScore, GaussianValidationLikelihood
...
class CachedHillClimbing(StructureEstimator):
    ...
    def __init__(self, data, scoring_method=None, blacklist=[], whitelist=[], **kwargs):
        ...
        if continuous and not isinstance(self.scoring_method, (GaussianBicScore, BGeScore, GaussianValidationLikelihood)):
    ...
    def score_add_arc(self, model, source, dest):
        ...
        return local_score(dest, parents_new) - \
    ...
    def estimate_parametric(
        self, start=None, tabu_length=0, max_indegree=None, epsilon=1e-4, max_iter=1e6, callbacks=[]
    ):
        """
        ...
        if max_indegree is None:
            best_operator_fun = lambda graph, scores: self.best_operator(graph, scores, epsilon)
        else:
            best_operator_fun = lambda graph, scores: self.best_operator_max_indegree(graph, scores, epsilon, max_indegree)
    ...
    def estimate_validation(
            self, start=None, tabu_length=0, max_indegree=None, epsilon=1e-4, max_iter=1e6, patience=0, callbacks=[]
    ):
        """
        This method runs the hill climbing algorithm.
        ...
        if max_indegree is None:
            best_operator_fun = lambda graph, scores: self.best_operator_validation(graph, scores, epsilon, tabu_last)
        else:
            best_operator_fun = lambda graph, scores: self.best_operator_max_indegree_validation(graph, scores, epsilon, max_indegree, tabu_last)
    ...
    def estimate(self, start=None, tabu_length=0, max_indegree=None, epsilon=1e-4, max_iter=1e6, patience=0, callbacks=[]):
        if isinstance(self.scoring_method, GaussianValidationLikelihood):
            return self.estimate_validation(start=start, tabu_length=tabu_length, max_indegree=max_indegree,
                                            ...
                                            callbacks=callbacks)
        elif isinstance(self.scoring_method, (GaussianBicScore, BGeScore)):
            return self.estimate_parametric(start=start, tabu_length=tabu_length, max_indegree=max_indegree, epsilon=epsilon,
                                    max_iter=max_iter, callbacks=callbacks)
